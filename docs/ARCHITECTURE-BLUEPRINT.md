# TiltCheck Architecture Blueprint
<!-- © 2024–2026 TiltCheck Ecosystem | v1.1.0 | Last Updated: 2026-02-26 -->
...
5. **Configure Secrets:**
   Create `.env` files for apps that need secrets (control-room needs Discord OAuth keys at minimum).
6. **Start with PM2:**
...
7. **Set up Reverse Proxy:**
   Set up Virtualmin reverse proxy rules as laid out earlier.

```bash
cp apps/dashboard/.env.example apps/dashboard/.env.local
cp apps/control-room/.env.example apps/control-room/.env
cp apps/casino-api/.env.example apps/casino-api/.env
cp apps/qualifyfirst-service/.env.example apps/qualifyfirst-service/.env
```

  Then fill in the real values. The one that needs the most attention is
   control-room — you'll need to go to the
  ...
  1. Create apps/api-gateway (the RGaaS pivot)
...
### 1. 🪙 JustTheTip (Tipping & Swaps)

- **The Logic Flow:** 1. User types `/tip @username 10 USDC` in Discord.

1. Bot checks the user's non-custodial Magic.link wallet balance.
2. Bot executes the transfer. It routes `10 USDC` to `@username` and routes the `[Flat Fee Amount]` to your environment variable `TREASURY_WALLET_ADDRESS`.
...
### 4. 📋 QualifyFirst (AI Survey Router)
...
### 6. 🧠 TiltCheck Core (Behavioral Risk)
