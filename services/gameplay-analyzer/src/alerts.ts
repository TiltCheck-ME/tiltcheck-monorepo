...
interface Alert {
  id: string;
  casinoId: string;
  type: 'pump' | 'compression' | 'clustering' | 'composite';
...
interface ThrottleConfig {
  cooldownMs: number; // per casino, per type
  dedupWindow: number; // consolidate similar alerts
}

interface EscalationConfig {
  multiAnomalyWindow: number; // time window for composite escalation
  criticalThreshold: number; // anomaly score for auto-escalate
}

class AlertManager {
  private recentAlerts: Map<string, number> = new Map(); // key: `${casinoId}:${type}`, value: lastTimestamp
  private alertHistory: Alert[] = [];
  private throttleConfig: ThrottleConfig;
  private escalationConfig: EscalationConfig;
  
  constructor(
    throttleConfig: ThrottleConfig = { cooldownMs: 5 * 60 * 1000, dedupWindow: 60 * 1000 },
    escalationConfig: EscalationConfig = { multiAnomalyWindow: 10 * 60 * 1000, criticalThreshold: 0.7 }
  ) {
   ...
   */
  emit(alert: Omit<Alert, 'id'>): Alert | null {
   ...
   */
  shouldEscalate(casinoId: string, currentAlert: Alert): boolean {
    const now = Date.now();
    const recentAnomalies = this.alertHistory.filter(a => 
      a.casinoId === casinoId &&
   ...
   */
  getRecentAlerts(casinoId: string, limit: number = 10): Alert[] {
   ...
   */
  resetThrottle(casinoId: string, type?: string) {
    if (type) {
...
export { AlertManager, Alert, ThrottleConfig, EscalationConfig };
