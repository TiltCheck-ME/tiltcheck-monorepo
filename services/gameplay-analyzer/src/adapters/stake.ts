...
function parseTimestamp(val: string | undefined): number {
...
function getColumnValue(row: Record<string, string>, keys: string | string[]): string | undefined {
  const keyArray = Array.isArray(keys) ? keys : [keys];
  for (const key of keyArray) {
    const normalized = Object.keys(row).find(k => k.toLowerCase() === key.toLowerCase());
    if (normalized && row[normalized]) return row[normalized];
  }
  return undefined;
}

export const stakeAdapter: CasinoAdapter = {
  id: 'stake-us',
  ...
  validate(row: Record<string, string>): boolean {
    ...
    if (win > bet * 10000) return false; // sanity: max 10000x multiplier
