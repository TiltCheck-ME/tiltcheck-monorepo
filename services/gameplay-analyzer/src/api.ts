...
export function startApiServer(casinoId: string) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (method === 'GET' && url.startsWith('/analyze')) {
      ...
      function serializeForSign(o: any) { return `${o.sessionId}|${o.userId}|${o.casinoId}|${o.expires}`; }
      function derivePublicKey(): Uint8Array | null {
        const pubHex = process.env.SESSION_PUBLIC_KEY?.trim();
        if (pubHex && pubHex.length === 64) return Buffer.from(pubHex, 'hex');
        const privHex = process.env.SESSION_SIGNING_SECRET?.trim();
        if (privHex && privHex.length === 64) return edGetPublicKey(Buffer.from(privHex, 'hex'));
        return null;
      }
      if (sessionToken) {
        try {
          ...
          if (!required.every(k => tokenObj && tokenObj[k])) {
            ...
            if (pub) {
              ...
              if (!ok) tokenError = 'Signature invalid';
            } else {
