...
interface SpinWindow {
  spins: Array<{ ts: number; bet: number; win: number; netWin: number }>;
  startIdx: number;
  endIdx: number;
}
...
export function detectPump(
  spins: Array<{ ts: number; bet: number; win: number }>,
  windowSize: number = 100,
  baselineRTP: number = 0.96,
  thresholdMultiplier: number = 1.15
): DetectionResult {
...
export function detectVolatilityCompression(
  spins: Array<{ ts: number; bet: number; win: number }>,
  compressionWindow: number = 50,
  comparisonWindow: number = 200,
  compressionThreshold: number = 0.3 // variance ratio threshold
): DetectionResult {
  ...
  const calcVariance = (data: typeof spins) => {
 ...
 */
export function detectWinClustering(
  spins: Array<{ ts: number; bet: number; win: number }>,
  clusterWindow: number = 20,
  winThresholdMultiplier: number = 1.5, // win > bet * multiplier = "win"
  clusterThreshold: number = 0.7 // % of wins in window to flag
): DetectionResult {
...
export function computeAnomalyScore(detections: {
  pump: DetectionResult;
  compression: DetectionResult;
  clustering: DetectionResult;
}): { score: number; severity: 'info' | 'warning' | 'critical'; flags: string[] } {
  const weights = { pump: 0.4, compression: 0.3, clustering: 0.3 };
  
  const score = 
    (detections.pump.detected ? detections.pump.confidence * weights.pump : 0) +
  ...
  const flags: string[] = [];
  if (detections.pump.detected) flags.push('pump');
  if (detections.compression.detected) flags.push('compression');
  if (detections.clustering.detected) flags.push('clustering');
  
  let severity: 'info' | 'warning' | 'critical' = 'info';
  if (score > 0.7) severity = 'critical';
  else if (score > 0.4) severity = 'warning';
