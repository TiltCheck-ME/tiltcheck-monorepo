...
function parseSession(raw) {
    try {
        ...
        if (!obj.sessionId || !obj.userId || !obj.casinoId || !obj.expires || !obj.signature)
            return null;
        return obj;
    }
    catch {
...
function serializeForVerify(info) {
...
wss.on('connection', ws => {
    ws.on('message', async (msg) => {
        try {
            ...
            const baseScore = typeof trust.anomalyScore === 'number' ? trust.anomalyScore : (typeof trust.severity === 'number' ? trust.severity : 0);
            const rtpDriftFrac = baselineRtp > 0 ? Math.abs(rtp - baselineRtp) / baselineRtp : 0;
            let rtpAdj = 0;
            if (rtpDriftFrac > 0.10)
                rtpAdj += 0.15;
            else if (rtpDriftFrac > 0.05)
                rtpAdj += 0.05;
            ...
            void eventRouter.publish('trust.casino.updated', 'gameplay-analyzer-ws', payload);
