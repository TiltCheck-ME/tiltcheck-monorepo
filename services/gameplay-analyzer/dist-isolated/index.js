...
import { adapterRegistry, stakeAdapter, rollbitAdapter } from './adapters/index.js';
...
const spins = [];
const spinIdSet = new Set(); // duplicate detection
...
const seedRotations = []; // minimal for engine; appended when seeds submitted
let activeAdapter;
...
export function normalizeSpin(raw) {
    const normalized = {
...
function computeBatchMetrics(batch) {
...
function computeRtpWindow(windowSize) {
    if (spins.length === 0)
        return { size: 0, meanNetWin: 0, deviationRatio: 0 };
...
function computeSeedStats() {
    if (seedRotations.length < 2)
        return { count: seedRotations.length, lastTs: seedRotations[seedRotations.length - 1]?.ts || null, avgIntervalMs: null };
...
function convertToEngineSpins(src) {
    return src.map(s => ({
        ts: s.ts,
        netWin: s.win - s.bet,
    }));
...
async function maybeGrade() {
    if (spins.length - lastGradeAtCount < GRADE_INTERVAL)
        return;
...
function detectRtpDrift(engineSpins) {
    if (engineSpins.length < GRADE_INTERVAL)
        return; // require minimum sample
...
function detectAnomalies() {
    if (spins.length < DETECTION_WINDOW)
        return;
...
export async function ingestCSV(path) {
...
function broadcastMetrics() {
    ...
    if (recentAnomalies.length) {
        ...
        const counts = recentAnomalies.reduce((acc, a) => { acc[a.type] = (acc[a.type] || 0) + 1; return acc; }, {});
...
eventRouter.publish = async function (type, source, data) {
    if (source === 'gameplay-analyzer') {
        if (type.startsWith('fairness.'))
            anomalyEventCount++;
        if (type === 'trust.casino.updated')
            gradingEventCount++;
    }
