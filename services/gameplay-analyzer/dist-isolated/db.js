...
`);
export function insertSpin(spin) {
...
export function insertSpinBatch(spins) {
    const stmt = db.prepare(`
    INSERT OR IGNORE INTO spins (id, casino_id, ts, bet, win, net_win, outcome)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
    const insertMany = db.transaction((batch) => {
        for (const s of batch)
            stmt.run(s.id, s.casino_id, s.ts, s.bet, s.win, s.net_win, s.outcome || null);
    });
    insertMany(spins);
}
export function insertSeed(seed) {
    const stmt = db.prepare(`
...
export function getSpinCount(casinoId) {
    const stmt = db.prepare('SELECT COUNT(*) as count FROM spins WHERE casino_id = ?');
    return stmt.get(casinoId).count;
}
export function getRecentSpins(casinoId, limit = 1000) {
    const stmt = db.prepare(`
    ...
    LIMIT ?
  `);
    return stmt.all(casinoId, limit);
}
export function getSeedRotations(casinoId) {
    const stmt = db.prepare(`
    ...
    ORDER BY ts ASC
  `);
    return stmt.all(casinoId);
}
export function getSpinsByTimeRange(casinoId, startTs, endTs) {
    const stmt = db.prepare(`
    SELECT id, casino_id, ts, bet, win, net_win, outcome
    FROM spins
    ...
    ORDER BY ts ASC
  `);
    return stmt.all(casinoId, startTs, endTs);
}
export function closeDb() {
    db.close();
}
export function insertAnomaly(a) {
    const stmt = db.prepare(`
...
export function getRecentAnomalies(casinoId, limit = 100) {
    const stmt = db.prepare(`
    ...
    LIMIT ?
  `);
    return stmt.all(casinoId, limit).map((r) => ({
        ...r,
