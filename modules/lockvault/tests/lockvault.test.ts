...
describe('LockVault Module', () => {
  ...
  it('locks a vault with valid inputs and emits vault.locked', async () => {
    const rec = await vaultManager.lock({ userId: 'u1', amountRaw: '5', durationRaw: '10m', reason: 'focus' });
  ...
  it('rejects lock below minimum duration', async () => {
    await expect(vaultManager.lock({ userId: 'u1', amountRaw: '2', durationRaw: '5m' })).rejects.toThrow(/Minimum lock/);
    expect(countEvents('vault.locked')).toBe(0);
  });

  it('prevents early unlock', async () => {
    const rec = await vaultManager.lock({ userId: 'u1', amountRaw: '1', durationRaw: '10m' });
