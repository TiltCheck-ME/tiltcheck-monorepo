 ...
 */
export function computeRtpZScore(observedRtp, claimedRtp, sampleSize) {
    if (sampleSize <= 0)
        return 0;
 ...
 */
export function computeRtpPValue(zScore) {
 ...
 */
export function computeMinSampleSize(claimedRtp, detectionDeltaPct, alpha = 0.05, power = 0.80) {
    if (detectionDeltaPct <= 0)
        return Infinity;
    const p0 = claimedRtp / 100;
 ...
 */
export function assessRtpConfidence(observedRtp, claimedRtp, sampleSize) {
    ...
    const evidenceSummary = buildEvidenceSummary(confidenceTier, delta, sampleSize, minimumRequiredSample, pValue, pPct, deltaPp);
    return {
        claimedRtp,
        observedRtp,
        sampleSize,
...
function buildEvidenceSummary(tier, delta, sampleSize, minimumRequired, pValue, pPct, deltaPp) {
    switch (tier) {
 ...
 */
export function computeProviderFairnessGrade(meanNerfDelta, nerfFrequency) {
    if (meanNerfDelta >= 5.0 || nerfFrequency >= 0.70)
        return 'F';
    if (meanNerfDelta >= 3.0 || nerfFrequency >= 0.50)
        return 'D';
    if (meanNerfDelta >= 2.0 || nerfFrequency >= 0.30)
        return 'C';
    if (meanNerfDelta >= 1.0 || nerfFrequency >= 0.20)
        return 'B';
    if (meanNerfDelta >= 0.5 || nerfFrequency >= 0.10)
        return 'A';
    return 'S';
}
