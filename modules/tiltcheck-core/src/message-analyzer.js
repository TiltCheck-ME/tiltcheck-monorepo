 ...
 */
export function analyzeMessages(messages, userId) {
    const signals = [];
...
function detectRapidMessaging(messages, userId) {
    const now = Date.now();
...
function detectCapsSpam(messages, userId) {
...
function detectRageKeywords(messages, userId) {
...
function detectLoanRequests(messages, userId) {
    const recentMessages = messages.slice(-5);
    for (const msg of recentMessages) {
        const analysis = analyzeSentiment(msg.content);
 ...
 */
export function calculateTiltScore(signals) {
    if (signals.length === 0)
        return 0;
    let weightedSum = 0;
    let totalWeight = 0;
 ...
 */
export async function analyzeMessagesWithAI(messages, userId, additionalContext) {
