 ...
 */
export function getNudgeMessage(signals) {
    if (signals.length === 0) {
    ...
    const appropriateMessages = messages.filter(m => {
        if (severityThreshold >= 0.8)
            return m.severity === 'firm' || m.severity === 'CRITICAL';
        if (severityThreshold >= 0.5)
            return m.severity === 'moderate' || m.severity === 'firm';
 ...
 */
export function getVibeCheckAlert(userId) {
 ...
 */
export function getEscalatedNudges(userId) {
    return [
 ...
 */
export function evaluateRtpLegalTrigger(rtpDelta, casinoName, licenseAuthority, confidence) {
    if (rtpDelta < 0.5)
        return null; // Below noise floor — no action
