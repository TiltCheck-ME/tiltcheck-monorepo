...
const cooldownHistory = new Map(); // userId -> violation timestamps
const DEFAULT_COOLDOWN_DURATION_MS = 15 * 60 * 1000; // 15 minutes
/**
 * Start a cooldown period for a user
 */
export function startCooldown(userId, reason, durationMs = DEFAULT_COOLDOWN_DURATION_MS) {
    const now = Date.now();
 ...
 */
export function endCooldown(userId) {
 ...
 */
export function recordViolation(userId) {
    const status = activeCooldowns.get(userId);
    if (!status)
        return;
    ...
    void eventRouter.publish('cooldown.violated', 'tiltcheck-core', {
        userId,
