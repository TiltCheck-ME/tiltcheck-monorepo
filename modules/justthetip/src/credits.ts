...
/**
 * JustTheTip - Credit Service (Custodial Credits)
 */
...
/**
 * CreditService handles custodial balances and transactions.
 * Extracted from apps/discord-bot to be shared.
 */
export class CreditService {
  ...
  async getBalance(discordId: string): Promise<CreditBalance | null> {
    if (this.db.isConnected()) {
      const balance = await this.db.getCreditBalance(discordId);
      return balance as CreditBalance | null;
    }
  ...
  async withdraw(discordId: string): Promise<{ amountLamports: number; walletAddress: string }> {
    ...
    if (!balance.wallet_address) {
      throw new Error('No wallet registered. Register your withdrawal wallet first.');
    }
  ...
  async getTransactionHistory(discordId: string, limit = 20): Promise<CreditTransaction[]> {
    if (this.db.isConnected()) {
      const txs = await this.db.getCreditTransactions(discordId, limit);
  ...
  async getStaleBalancesForRefund(): Promise<CreditBalance[]> {
    if (this.db.isConnected()) {
      ...
      return allStale as CreditBalance[];
    }
  ...
  private loadFallback(): void {
    try {
      ...
      console.warn('[CreditService] Failed to load fallback:', err);
    }
  }
  ...
  private saveFallback(): void {
    try {
      ...
      console.error('[CreditService] Failed to save fallback:', err);
    }
  }
  ...
  private fbDebit(discordId: string, amount: number, type: string): number {
    ...
    if (type === 'tip_send') fb.totalTipped += amount;
    if (type === 'withdraw') fb.totalWithdrawn += amount;
