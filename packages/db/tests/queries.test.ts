...
describe('@tiltcheck/db users fallback', () => {
  ...
  it('retries createUser without discord_avatar when Supabase users schema is behind', async () => {
    ...
    expect(fallbackClient.insert.mock.calls[0][0]).toMatchObject({ discord_avatar: 'avatar-123' });
    expect(fallbackClient.insert.mock.calls[1][0]).not.toHaveProperty('discord_avatar');
    warnSpy.mockRestore();
  });
