...
import { TiltDetector } from '../src/tilt-detector.js';
import { CasinoLicenseVerifier } from '../src/license-verifier.js';
 ...
 */
function initialize() {
  console.log('[TiltGuard] Initializing...');
  ...
  // Notify user if unlicensed
  if (!casinoVerification.shouldAnalyze) {
    showPersistentWarning(casinoVerification.warningMessage!);
  }
...
async function startMonitoring() {
  ...
  // Don't analyze unlicensed casinos
  if (!casinoVerification?.shouldAnalyze) {
    showNotification('❌ Cannot analyze unlicensed casino', 'error');
    return;
  }
  ...
  try {
    ...
    showNotification('⚠️ Analyzer backend offline - tilt monitoring only', 'warning');
  }
  ...
  isMonitoring = true;
  showNotification('🛡️ TiltGuard Active', 'success');
  
  // Start periodic tilt checks
  startTiltMonitoring();
}
...
// ... rest of helper functions (showNotification, getUserId, detectCasinoId, etc.) ...
// (Reuse from previous content.ts)
