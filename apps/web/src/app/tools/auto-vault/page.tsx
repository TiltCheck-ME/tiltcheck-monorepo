        ...
        <div className="max-w-4xl mx-auto px-4 py-12 pt-32">
          <div className="mb-10 rounded-3xl border border-[#17c3b2]/25 bg-black/40 p-8 md:p-10">
            ...
            <p className="text-gray-400 text-sm max-w-2xl">
              Web still explains the system. The actual AutoVault rules now live in the canonical user-dashboard beside
              LockVault timers, Wallet Lock cooldowns, and release controls.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              >
                {loading ? 'Checking session...' : user ? 'Open AutoVault rules' : 'Log in for AutoVault rules'}
              </a>
              <a
                href={vaultLaneUrl}
            ...
            {user && (
              <p className="mt-4 text-xs font-mono text-gray-500">
                Session found. Redirecting you to the canonical AutoVault lane now.
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            {[
              { step: '01', label: 'Set the rule', desc: 'Define the trigger that should auto-send wins or overages into LockVault.' },
              { step: '02', label: 'Auto-vault on trigger', desc: 'Dashboard writes the durable AutoVault rule. Web is not running a shadow rules engine.' },
              { step: '03', label: 'Review the lock', desc: 'LockVault timers, staged unlocks, and Wallet Lock requests stay in the same canonical dashboard lane.' },
            ].map(({ step, label, desc }) => (
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              ...
              <div className="rounded-2xl border border-[#17c3b2]/25 bg-[#17c3b2]/5 p-5">
                ...
                <p className="mt-3 text-sm text-gray-300">
                  Live AutoVault rules, LockVault state, Wallet Lock cooldowns, and unlock requests are managed in the canonical dashboard.
                </p>
              </div>
            </div>
          </div>
