...
export default function NutsSetupClient() {
  ...
  React.useEffect(() => {
    ...
    setScriptUrl(resolveNutsScriptUrl(origin));
    setPageUrl(resolveNutsPageUrl(origin));
    setChromeWarning(isAndroidChrome());
  }, []);

  const tracks = buildNutsTracks(scriptUrl || resolveNutsScriptUrl());
    ...
    <div className="max-w-md mx-auto px-4 py-8 pb-16">
      <header className="mb-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-black text-white leading-tight mb-3">
          Auto-lock wins to vault
        </h1>
        <p className="text-sm text-gray-400 leading-relaxed">
          On a heater? This skims part of each win to vault so you do not rinse it all back.
          Free. You flip it ON or OFF anytime.
        </p>
      </header>
      ...
      <section className="rounded-2xl border border-[#283347] bg-black/30 p-5 mb-8">
        <p className="text-[10px] font-mono font-black uppercase tracking-[0.16em] text-gray-500 mb-2">
          Send a friend (DM only — no links in nuts chat)
        </p>
        <p className="text-xs text-gray-400 mb-3 leading-relaxed">
          Copy this and paste in a private message. Do not spam public chat — you will get muted.
        </p>
        <textarea
          readOnly
          value={pageUrl ? NUTS_DM_BLURB.replace(NUTS_SETUP_PAGE_PRODUCTION, pageUrl) : NUTS_DM_BLURB}
          rows={3}
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            onClick={() =>
              copyText(
                pageUrl ? NUTS_DM_BLURB.replace(NUTS_SETUP_PAGE_PRODUCTION, pageUrl) : NUTS_DM_BLURB,
                'dm'
              )
            }
          ...
          <button
            type="button"
            onClick={() => copyText(pageUrl || NUTS_SETUP_PAGE_PRODUCTION, 'link')}
